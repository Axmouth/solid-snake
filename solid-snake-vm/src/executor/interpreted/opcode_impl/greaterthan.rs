    ...
    ($opcode:ident, $ty:ty) => {
        paste! {
            $crate::define_instruction!(
                $opcode,
                concat!("Compares two ", stringify!($ty), " registers. Sets 1 if the first is greater than the second, else 0."),
                [
...
macro_rules! impl_greaterthan_float_instruction {
    ($opcode:ident, $ty:ty) => {
        paste! {
            $crate::define_instruction!(
                $opcode,
                concat!("Compares two ", stringify!($ty), " floating-point registers. Sets 1 if the first is greater than the second, else 0."),
                [
                    (dest: RegisterType, "Destination register for result (1 or 0)"),
                    (reg1: RegisterType, "First operand (left-hand side)"),
                    (reg2: RegisterType, "Second operand (right-hand side)")
